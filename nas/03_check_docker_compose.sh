...
if ! [[ $result == *"version"* ]]; then    # 如果不包含"version"字符串，则安装docker-compose  
  ...
  wget https://d.nastool.work/d/aliyun/Cloud%20Sync/1.Alist/wget/nastool/docker-compose-linux-x86_64.gz -O /usr/local/bin/docker-compose && sudo chmod +x /usr/local/bin/docker-compose && docker-compose --version
